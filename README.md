- 0.2.44: Fixed the hosted-bootstrap fallback plumbing for scheduler scaffold bring-up. `QueryMapping` fallback is now reachable instead of fatal-halting on the single hosted-step attempt, fallback completions are counted only once, local `MapPages`/`UnmapPages` now resolve the target address-space object and use its real PML4 root, local address-space resolution now accepts ready service-created objects without hard-binding to bootstrap signature/version constants, and the bootstrap dispatcher now guards null inputs before zeroing the response buffer. Also tightened hosted service-step readiness so a task already marked RUNNING cannot be re-entered.
- 0.2.43: Added kernel-side hosted-bootstrap fallbacks for `AttachStagedImage` and `AllocateAddressSpaceStack`. If the hosted memory-manager pump stops publishing a real reply while the scheduler scaffold is attaching its first user image or allocating the first user stack, the kernel now stages those mappings directly into the target address space instead of fatal-halting during the ring-transition bring-up.
- 0.2.42: Added a kernel-side `QueryMapping` fallback for the hosted memory-manager bootstrap path. When the hosted service pump does not publish a reply during scheduler scaffold probes, the kernel now resolves the target address-space object directly and walks its page tables itself, so unmapped/mapped user probes no longer stall bootstrap waiting for a reply that never arrives.
- 0.2.41: Fixed the hosted memory-manager request pump for the scheduler user-transition path. Kernel-side memory-manager round-trips now run with interrupts held off so hosted service replies cannot be lost to re-entrant bootstrap traffic, and `QueryMapping` is now a non-destructive probe for unmapped user addresses instead of treating an unmapped low-half address as a hard-fail condition. This unblocks the scaffold image/stack mapping phase that 0.2.40 could halt during.
- 0.2.40: Wired the first real x64 ring-transition substrate into the scheduler user scaffold. The kernel now installs a real TSS descriptor and loads TR, refreshes TSS.RSP0 before user dispatch, maps a staged user ELF plus real user stack into the scaffold process's owned address space through the memory manager, and switches the scheduler bridge to a true `iretq` entry path. A DPL3 vector 128 gate now proves the ring-3 -> ring-0 return path, and the scheduler consumes that user-origin trap by terminating and reaping the scaffold task instead of halting the kernel.
- 0.2.39: Scheduler serial output is now held back until the user-transition scaffold reaches `seal-ready`, so the first scheduler-side serial lines appear only once the staged user handoff metadata is fully formed. Added an explicit scheduler-side `complete` stage after `live-gate-closed`; heartbeat/state output now exposes `user-scaffold-complete`, and the scaffold process/task now move to `user-state=15` once the scheduler-side handoff preparation is fully finished while real user-mode dispatch still remains gated behind a future TSS/true ring-transition implementation.
- 0.2.38: Added an explicit handoff-ready stage after `seal-ready` for the user-transition scaffold. The scheduler now freezes the final blocked-task handoff stack pointer as non-zero `user-handoff-sp` metadata, exposes `user-scaffold-handoff-ready` in heartbeat output, and requires that final saved stack pointer before the live gate can close or a future real ring-transition path can mark the scaffold live.
- 0.2.36: Added an explicit contract-ready stage after `chain-ready` for the user-transition scaffold. The scheduler now verifies the staged frame and dispatch chain, freezes a non-zero `user-contract` signature over that metadata, and exposes `user-scaffold-contract-ready` in heartbeat output. The live gate still remains closed until the real ring-transition entry path exists, and both live-gating and future `LIVE` promotion now require the prepared contract metadata to be present.
- 0.2.35: Added an explicit chain-ready stage after `bridge-ready` for the user-transition scaffold. The scheduler now stages a dedicated non-live dispatch chain on the blocked scaffold task's kernel stack, records the non-zero `user-chain-sp` in process/task traces, and exposes `user-scaffold-chain-ready` in heartbeat output. The live gate still remains closed and `LosKernelSchedulerMarkUserTransitionScaffoldLive()` now requires that prepared chain metadata to exist before a future real ring-transition handoff can mark the scaffold live.
- 0.2.32: Added an explicit frame-ready stage after `descriptor-ready` for the user-transition scaffold. The scheduler now writes a future `iretq` return frame template onto the blocked scaffold task's kernel stack, records the non-zero `user-frame-sp` in process/task traces, and exposes `user-scaffold-frame-ready` in heartbeat output. The scaffold still remains live-gated and blocked, and `LosKernelSchedulerMarkUserTransitionScaffoldLive()` now requires that prepared frame metadata to exist before a future real ring-transition handoff can mark the scaffold live.
- 0.2.31: Added an explicit descriptor-ready stage after `entry-ready` for the user-transition scaffold. The kernel GDT now carries user code/data descriptors, scheduler traces now record future user CS/SS/RFLAGS values, and scheduler heartbeats now expose `user-scaffold-descriptor-ready` while the scaffold still remains live-gated and blocked until the real ring-transition handoff exists.
